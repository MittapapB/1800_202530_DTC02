...
export async function fetchUserComments() {
  ...
  try {
    ...
    const promises = snapshot.docs.map(async (docSnap) => {
      ...
      let visitTime = "-";
      if (record.visit_timestamp?.toDate) {
        const dateObj = record.visit_timestamp.toDate();
    ...
    results.forEach(({ record, user, visitTime }, index) => {
      ...
      if (hasRating) {
        const maxStars = 5;
        const filledStars = record.rating;
        starsHtml = Array.from({ length: maxStars }, (_, i) => {
      ...
      card.innerHTML = `
        <img src="${avatarUrl}" alt="User avatar"
          class="w-12 h-12 rounded-full object-cover border border-gray-200 user-avatar" />
          ...
          <div class="flex justify-between items-center">
            ...
            <p class="text-sm text-gray-500 font-semibold flex items-center gap-1">
              <svg xmlns="http://www.w3.org/2000/svg"
                class="w-5 h-5 text-[#EB6424]" fill="none" viewBox="0 0 24 24"
